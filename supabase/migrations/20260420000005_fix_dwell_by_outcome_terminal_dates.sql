-- Fix: use stage-specific exit dates for terminal outcomes so dwell reflects
-- true "in 3rd Call" time, not post-close modification activity.
...
as $func$
  ...
  pairs as (
    select
      ...
      extract(epoch from (
        case current_stage_id
          when 'closedwon'    then date_entered_closedwon
          when 'closedlost'   then date_entered_closedlost
          when 'contractsent' then date_entered_contractsent
          when '1066871403'   then date_entered_1066871403
          when '1066193534'   then coalesce(last_activity_at, now())
          else null::timestamptz
        end
        - date_entered_1066193534
      )) / 86400.0 as days
    from latest
