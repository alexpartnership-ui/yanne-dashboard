...
export function RepsPage() {
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        {sorted.map(rep => {
          ...
          return (
            <div
              key={rep.id}
              className={`rounded-lg border border-border bg-surface-raised shadow-sm hover:shadow-md transition-shadow border-l-4 ${borderColor}`}
            >
              {/* Header */}
              <div className="flex items-center justify-between px-6 pt-5 pb-3">
                <div className="flex items-baseline gap-3">
                  <h3 className="text-2xl font-bold text-text-primary">{rep.rep}</h3>
                  <span className="rounded-full bg-surface-overlay px-3 py-0.5 text-xs font-medium text-text-muted">
                    {rep.total_scored_calls} calls scored
                  </span>
                </div>
                <div className="tooltip-wrap relative text-xs text-text-faint">
                  Qualification rate: <span className="font-semibold text-text-secondary">{(rep.qualification_rate * 100).toFixed(0)}%</span>
                  <div className="tooltip-text absolute right-0 bottom-full mb-1 z-50 w-64 rounded-lg bg-surface px-3 py-2 text-xs text-white shadow-lg">
              ...
              <div className="border-t border-border-muted px-6 py-4 flex gap-8 items-start">
                <div className="flex gap-8 text-sm shrink-0">
                  <div>
                    <span className="text-[11px] text-text-faint uppercase tracking-wider block mb-0.5">Strongest</span>
                    <span className="font-medium text-text-primary">{rep.strongest_category ?? '\u2014'}</span>
                  </div>
                  <div>
                    <span className="text-[11px] text-text-faint uppercase tracking-wider block mb-0.5">Weakest</span>
                    <span className="font-medium text-text-primary">{rep.weakest_category ?? '\u2014'}</span>
                  </div>
                </div>
      ...
      {adherence.length > 0 && (
        <div className="mt-8">
          <h3 className="text-sm font-bold text-text-primary mb-3">Coaching Accountability</h3>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            {adherence.map(a => (
              <div key={a.rep} className={`rounded-lg border bg-surface-raised p-4 shadow-sm ${a.repeatingIssue ? 'border-red-200' : 'border-border'}`}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-bold text-text-primary">{a.rep}</span>
                ...
                {a.currentFocus && (
                  <div className="mb-2">
                    <span className="text-[10px] text-text-faint uppercase">Current Focus:</span>
                    <p className="text-xs text-text-muted mt-0.5 line-clamp-2">{a.currentFocus}</p>
                  </div>
                )}
                {a.repeatingIssue && (
                ...
                <div className="flex items-center gap-1 mt-2">
                  <span className="text-[10px] text-text-faint">Last 5:</span>
