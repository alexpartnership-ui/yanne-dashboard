...
function SequenceViewer({ campaignId }: { campaignId: number }) {
  ...
  if (steps.length === 0) return <div className="p-3 text-xs text-text-faint">No sequence data</div>
  ...
  return (
    <div className="space-y-2">
      {mainSteps.map((step, i) => {
        ...
        return (
          <div key={step.id} className="rounded-lg border border-border-muted bg-surface-raised overflow-hidden">
            <button
              onClick={() => setExpandedStep(isExpanded ? null : step.id)}
              className="flex w-full items-center justify-between px-4 py-3 hover:bg-surface-raised transition-colors text-left"
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs font-medium text-text-primary">{step.email_subject}</div>
              ...
              <svg className={`w-4 h-4 text-text-faint transition-transform ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
            ...
            {isExpanded && (
              <div className="px-4 pb-4 border-t border-border-muted">
                {/* Main step body */}
                <div className="mt-3">
                  <div className="text-[10px] font-semibold text-text-muted uppercase tracking-wider mb-1">Email Body</div>
                  <pre className="rounded bg-surface-raised p-3 text-xs text-text-secondary whitespace-pre-wrap font-sans leading-relaxed max-h-[200px] overflow-y-auto border border-border-muted">
                ...
                {variants.map((v, vi) => (
                  <div key={v.id} className="mt-3 rounded-lg border border-dashed border-violet-200 bg-violet-50/50 p-3">
                    ...
                    <pre className="rounded bg-surface-raised p-3 text-xs text-text-secondary whitespace-pre-wrap font-sans leading-relaxed max-h-[150px] overflow-y-auto border border-violet-100">
...
export function ActiveCampaignsPage() {
  ...
  return (
    <div>
      <h2 className="mb-4 text-2xl font-bold text-text-primary">Active Campaigns</h2>
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        <span className="text-xs text-text-faint">{activeCampaigns.length} active campaigns</span>
      </div>
      ...
      <div className="flex gap-4">
          ...
          {activeCampaigns.map(c => (
            <button
              ...
              className={`w-full text-left rounded-lg border p-4 transition-all hover:shadow-md ${
                selectedCampaign?.id === c.id ? 'border-yanne bg-yanne/5 shadow-md' : 'border-border bg-surface-raised shadow-sm'
              }`}
            >
              <div className="text-sm font-medium text-text-primary mb-1 leading-tight">{c.name}</div>
              ...
              <div className="text-[9px] text-text-faint mt-0.5">{c.completion_percentage}% complete</div>
            </button>
          ))}
        </div>
        ...
        <div className="flex-1">
          {!selectedCampaign ? (
            <div className="rounded-lg border-2 border-dashed border-border bg-surface-raised py-20 text-center text-sm text-text-faint">
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-lg border border-border bg-surface-raised p-5 shadow-sm">
                <h3 className="text-lg font-bold text-text-primary mb-4">{selectedCampaign.name}</h3>
                <div className="grid grid-cols-4 gap-4">
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Emails Sent</div>
                    <div className="text-xl font-bold text-text-primary">{selectedCampaign.emails_sent.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Replies</div>
                    <div className="text-xl font-bold text-text-primary">{selectedCampaign.replied}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Interested</div>
                    <div className="text-xl font-bold text-emerald-600">{selectedCampaign.interested}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Bounced</div>
                    <div className={`text-xl font-bold ${selectedCampaign.bounce_rate > 3 ? 'text-red-600' : 'text-text-primary'}`}>{selectedCampaign.bounced} ({selectedCampaign.bounce_rate.toFixed(2)}%)</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Total Leads</div>
                    <div className="text-xl font-bold text-text-primary">{selectedCampaign.total_leads.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Contacted</div>
                    <div className="text-xl font-bold text-text-primary">{selectedCampaign.total_leads_contacted.toLocaleString()}</div>
                  </div>
                  <div>
                    <div className="text-[10px] text-text-muted uppercase">Completion</div>
                    <div className="text-xl font-bold text-text-primary">{selectedCampaign.completion_percentage}%</div>
                  </div>
                </div>
              </div>
