...
export function CallDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      {/* Back link */}
      <Link to="/calls" className="text-sm text-zinc-500 hover:text-zinc-700">&larr; Back to calls</Link>
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            {call.prospect_contact && (
              <p className="mt-1 text-sm text-zinc-500">Contact: {call.prospect_contact}</p>
            )}
          </div>
          <div className="text-right">
            <div className="flex items-center gap-3">
              <span className="text-3xl font-bold text-zinc-900">{call.score_percentage}%</span>
              <GradeBadge grade={call.grade} />
            </div>
            <p className="mt-1 text-xs text-zinc-500">{call.total_score} / {call.max_possible} pts</p>
          </div>
        </div>
        {call.gdrive_doc_url && (
          <a
      ...
      {categories.length > 0 && (
        <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
          <h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-zinc-500">Category Scores</h3>
          <div className="space-y-3">
            {categories.map((cat, i) => (
              <div key={i}>
                <div className="mb-1 flex items-center justify-between text-sm">
                  <span className="text-zinc-700">{cat.category}</span>
                  <span className="font-medium text-zinc-900">{cat.score}/{cat.max}</span>
                </div>
              ...
              <ul className="space-y-2">
                {penalties.map((p, i) => (
                  <li key={i} className="text-sm">
                    ...
                    <span className="ml-2 text-zinc-700">{p.name}</span>
                    {p.reason && <p className="ml-2 text-xs text-zinc-500">{p.reason}</p>}
                  </li>
                ))}
              </ul>
            </div>
              ...
              <ul className="space-y-2">
                {bonuses.map((b, i) => (
                  <li key={i} className="text-sm">
                    ...
                    <span className="ml-2 text-zinc-700">{b.name}</span>
                    {b.reason && <p className="ml-2 text-xs text-zinc-500">{b.reason}</p>}
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        {call.biggest_miss && (
          <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
            ...
            <p className="text-sm text-zinc-700">{call.biggest_miss}</p>
          </div>
        )}
      </div>
      ...
      {call.coaching_priority && (
        <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
          <h3 className="mb-2 text-sm font-semibold uppercase tracking-wider text-zinc-500">Coaching Priority</h3>
          <p className="text-sm text-zinc-700">{call.coaching_priority}</p>
        </div>
      )}
      ...
      {call.qualification_result && (
        <div className="rounded-lg border border-zinc-200 bg-white p-6 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-zinc-500">Qualification</h3>
      ...
      {call.full_scorecard_text && (
        <div className="rounded-lg border border-zinc-200 bg-white shadow-sm">
          <button
            ...
            className="w-full px-6 py-4 text-left text-sm font-semibold uppercase tracking-wider text-zinc-500 hover:bg-zinc-50 transition-colors"
          >
            {showFull ? 'Hide' : 'Show'} Full Scorecard
          </button>
          {showFull && (
            <pre className="max-h-[600px] overflow-auto border-t border-zinc-200 px-6 py-4 text-xs text-zinc-700 whitespace-pre-wrap">
              {call.full_scorecard_text}
            </pre>
          )}
        </div>
